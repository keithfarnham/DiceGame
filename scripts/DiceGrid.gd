...
#@onready var dieFaceGrid = $DiceGridControl/DiceGridContainer/Panel/AspectRatioContainer/HBoxContainer/FaceScrollContainer/FaceGrid as GridContainer
...
func add_die(die : Die, dieIndex : int):
	...
	print("die type is " + str(DiceData.DiceTypes.keys()[die.type]))
	match die.type:
	...
	add_child(newDieUIInstance)

func add_dice(dice : Array[Die]):
	
	for dieIndex in dice.size():
		#setting up instance of DieUI
		print("adding dice at index " + str(dieIndex) + " of type " + str(DiceData.DiceTypes.keys()[dice[dieIndex].type]))
...
func die_selected(dieIndex : int):
	...
	print("die_selected prev selected " + str(prevSelect) + " with die index " + str(dieIndex) + " and dieFaceGrid node " + str(dieFaceGrid))
	...
	if prevSelect == dieIndex and dieFaceGrid.visible:
		...
		get_child(dieIndex).release_focus()
	else:
	...
	print("sceneroot name " + str(sceneRoot.name))
	if sceneRoot.name == "RollReward":
		reward = true
	for faceIndex in (sceneRoot.rewardDice[dieIndex].faces.size() if reward else PlayerDice.ScoreDice[dieIndex].num_faces()):
		...
		typeNode.text = str(DieFaceData.FaceType.keys()[sceneRoot.rewardDice[dieIndex].faces[faceIndex].type] \
		...
		valueNode.text = str(DieFaceData.RewardTypeValue.keys()[sceneRoot.rewardDice[dieIndex].faces[faceIndex].value] \
...
func clear_die_face_grid():
	for child in get_parent().get_parent().get_node('FaceScrollContainer').get_node('FaceGrid').get_children():
		get_parent().get_parent().get_node('FaceScrollContainer').get_node('FaceGrid').remove_child(child)
